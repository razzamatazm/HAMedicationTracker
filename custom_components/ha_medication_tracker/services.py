...
RECORD_DOSE_SCHEMA = vol.Schema(
    {
        vol.Required(ATTR_MEDICATION_ID): cv.string,
        vol.Optional(
            ATTR_DOSE_TIMESTAMP, default=lambda: dt_util.utcnow().isoformat()
        ): cv.string,
...
RECORD_TEMPERATURE_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(
            ATTR_TEMPERATURE_TIMESTAMP, default=lambda: dt_util.utcnow().isoformat()
        ): cv.string,
...
async def async_setup_services(hass: HomeAssistant) -> None:
    ...
    async def async_handle_remove_patient(call: ServiceCall) -> None:
        ...
        if result:
            ...
            _LOGGER.warning(
                "Failed to remove patient with ID %s - not found", patient_id
            )
    ...
    async def async_handle_remove_medication(call: ServiceCall) -> None:
        ...
        if result:
            ...
            _LOGGER.warning(
                "Failed to remove medication with ID %s - not found", medication_id
            )
    ...
    async def async_handle_record_dose(call: ServiceCall) -> None:
        ...
        if result:
            ...
            _LOGGER.warning(
                "Failed to record dose for medication ID %s - not found", medication_id
            )
    ...
    async def async_handle_record_temperature(call: ServiceCall) -> None:
        ...
        if result:
            ...
            _LOGGER.warning(
                "Failed to record temperature for patient ID %s - not found", patient_id
            )
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_REMOVE_PATIENT,
        async_handle_remove_patient,
        schema=REMOVE_PATIENT_SCHEMA,
    )

    hass.services.async_register(
        DOMAIN,
        SERVICE_ADD_MEDICATION,
        async_handle_add_medication,
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_REMOVE_MEDICATION,
        async_handle_remove_medication,
    ...
    hass.services.async_register(
        DOMAIN,
        SERVICE_RECORD_TEMPERATURE,
        async_handle_record_temperature,
        schema=RECORD_TEMPERATURE_SCHEMA,
    )
